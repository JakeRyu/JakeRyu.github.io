...
이전 글, [ASP.NET Core로 구현하는 클린 아키텍쳐 (1)]({% post_url 2020-04-05-api-버전관리 %})에서 정리한 클린 아키텍쳐에 대한 개념을 확실히 잡고 이 글을 시작하기 바란다.
...
요구사항 중에, *`Payment` 엔티티를 저장하면서 카드번호도 함께 저장하는데 조회시에는 카드번호를 전부 보여주지 않는다(마스킹)* 라는 요건이 있다면 어디에서 이 요건을 구현하는 것이 가장 좋을까?

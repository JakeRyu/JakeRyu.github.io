...
윈도우가 아닌 다른 운영체제에서의 현실적인 대안은 `Visual Studio Code` 이다. 무료이고 익스텐션 생태계가 풍부해서 마이크로소프트가 놀랄 정도로 시장에 잘 받아들여진 제품이다. 그리고, 꾸준이 업데이트 되고 있고 익스텐션 지속적으로 증가하고 있다.
...
VS Code는 텍스트 에디터다. 솔루션, 프로젝트 생성 등 IDE에서 당연시했던 기능들이 없다. 대신, 외부 명령어를 실행해야 하는데 사실 이것은 비주얼스튜디오의 작동원리와도 동일하다. 외부 명령에 해당하는 UI가 제공하는 차이가 있을뿐이다. 커맨드 명령에 익숙해져야 하고 터미널 창을 사용하는 시간이 늘어날테지만 더욱 개발자스러워지고 있다고 생각하자.
...
`docker-dev-image`라는 이미지명을 기억하고 다음 단계에서 컨테이너를 만들어 보자.
...
`postgres-weather`라는 이름의 리포지토리를 생성한다. 이 과정은 생략할 수도 있는데 이미지를 게시할 때, 이미지 이름에 해당하는 리포지토리를 자동으로 생성해 주기 때문이다. 이미지 이름에 `/`를 사용하여 네임스페이스와 리포지토리를 구분하다. 이 곳에 올라온 이미지는 `jakecompany/<image name>`로 접근할 수 있고, 반대로 이미지 이름을 이와 같이 작성하여 `push`하면 자동으로 네임스페이스를 찾아간다.
